...
kiteResult=IIFLLogin.loginEasy(IIFLUSERID,userExcelFile)
KITE=None

if kiteResult.get('status'):
    KITE=kiteResult.get('kite')
    logging.debug(f'Login Successful {IIFLUSERID}')
else:
    logging.debug(f'Login failed {IIFLUSERID} = {kiteResult.get("msg")}')
    print('Unable to Login IIFL Id')
    exit()
...
def liveDataStart():
    global liveTicker
    loginResult=IIFLLogin.loginEasy(IIFLUSERID,userExcelFile,tickerFlag=1)
        ...
        try:
            ...
            if status=='COMPLETE':
                ...
                if str(data.get('status_message','')).find('execution range')!=-1:
                    ...
                    threading.Thread(target=IIFLServices.placeOrder,args=(data,10)).start()
        except Exception as e:
...
def orderPlaceHigh(request):
    ...
    # print(IIFLUSERID)
    if request.get('data').get('userId')!=IIFLUSERID:
        response={'processId':request.get('processId',-1) ,'status':0,'msg':'IIFL Users are different for server and client'}
    ...
    if qty!=0:
        ...
        response=IIFLServices.placeOrder(item)
        logging.debug(f'Response Place Order Function {response}')    
...
def placeRawOrder(request):
    ...
    item=request.get('data')
    item['kite']=KITE
    orderResult=IIFLServices.placeOrder(item)
...
def cancelAndRecreateOrder(orderId):
    ...
    result=IIFLServices.placeOrder(orderData)
    print(result)
    return result
