        ...
        try:
            ...
            if find==1:
                ...
                if command in ['0',0,'c']:
                    ...
                    print(result)
                    baseResult=radheUtils.search(item.get('baseLTP'))
                    # print(item.get('instrument'))
                    # print(item.get('base'))
                    # print(result)
                    if result==0 or baseResult==0 or item.get('firstTrade') not in ['BUY','SELL']:
                        invalidDataFlag=1
                    else:
                        if type(result)==dict:
                            item['ltpDual']=result
                            item.update(result) #For Place Order Function  This will act as raw material. This order is placed
                            item['ltpKey']=f'{result.get("exchangeInstrumentID")}{result.get("exchangeSegment")}'
                            item['baseDual']=baseResult
                            item['baseKey']=f'{baseResult.get("exchangeInstrumentID")}{baseResult.get("exchangeSegment")}'
                        else:
                            invalidDataFlag=1
                    print(item)
    ...
    if isTickerConnected:
        ...
        if temp.find(':')!=-1:
            ...
            if floatFlag==1:
                ...
                subscriptionList.append(item.get('baseDual'))
                subscribe(item.get('baseDual'))
                ...
                if radheUtils.low(item.get('checkPriceBase'))=='ask-bid':
                    def CheckFunction():
                        ltp=tokenDict.get(item.get('baseKey'),{}).get('depthS')
                        return (ltp>=Lcheck and ltp<=Ucheck) or dictObject.get('stopFlag')
                else:
                    def CheckFunction():
                        ltp=tokenDict.get(item.get('baseKey'),{}).get('ltp')
                        return (ltp>=Lcheck and ltp<=Ucheck) or dictObject.get('stopFlag')
...
def priceOrderHeart(item,dictObject,transaction):
    ...
    if transaction=='BUY':
        ...
        def buyCon():
            ltp=item.get('buyLtpFunction')()#tokenDict.get(item.get('instrument_token')).get('ltp')
            print(f"offer price {ltp} {item.get('serial')}")
        ...
        def sellCon():
            ...
            print(f"bid price {ltp} {item.get('serial')}")
