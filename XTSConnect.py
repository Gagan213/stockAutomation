...
class XTSConnect(XTSCommon):
    ...
    # cfg.read('..\XTConnect_2\config.ini')

    # Default root API endpoint. It's possible to
    # override this by passing the `root` parameter during initialisation.
    _default_root_uri = 'https://ttblaze.iifl.com'  #cfg.get('root_url', 'root')
    _default_login_uri = _default_root_uri + "/user/session"
    _default_timeout = 7  # In seconds

    # SSL Flag
    _ssl_flag = True #cfg.get('SSL', 'disable_ssl')
    ...
    def interactive_login(self):
        ...
        try:
            ...
            return {'status':0,'msg':response.get('description')}
    ...
    def place_order(self,item):
        """To place an order"""
        try:
            print(f'Placing Order Final Function {datetime.datetime.now()}')
            print(item)
            while not item.get('semaphoreOrderAPI').acquire(blocking=False):
                print(f"I am inside semaphore loop {item.get('excelRowId')}")
                time.sleep(1) # For Order API Time Limit
                
            
            params = {
                    'exchangeSegment':item.get('exchangeSegment'),
                    'exchangeInstrumentID':item.get('exchangeInstrumentID'),
                    'productType':item.get('product'),
                    'orderType':item.get('orderType'),
                    'orderSide':item.get('transaction_type'),
                    'timeInForce':item.get('validity'),
                    'disclosedQuantity':item.get('disclosedQuantity',0),
                    'orderQuantity':item.get('quantity'),
                    'limitPrice':item.get('price',0),
                    'stopPrice':item.get('stopPrice',0),
                    'orderUniqueIdentifier':str(item.get('orderUniqueIdentifier',random.randint(1,5000))),
            ...
            # print(f'inside final order function {response}')
            if response.get('type')=='success':
                return {'status':1,'orderId':response.get('result',{}).get('AppOrderID',0)}
            elif response.get('type')=='error':
                print(f'error response received from server while placing order {response}')
                return {'status':0,'msg':response.get('description')}
        except Exception as e:
            print(f'{e} error in order placing {item}')
            try:
                msg=response.get('description')
            except:
                msg=e
            return {'status':0,'msg':f'Error while placing order - {msg}'}
        finally:
            item.get('semaphoreOrderAPI').release()
    ...
    def send_subscription_symbol(self, isTradeSymbol, Instruments, xtsMessageCode):
        try:
            params = {'isTradeSymbol': isTradeSymbol, 'instruments': Instruments, 'xtsMessageCode': xtsMessageCode}
